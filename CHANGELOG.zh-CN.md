...
- 按天同步不再把「本地只剩某一天」误当成删除其余天；刷新时若核心 snapshot 里 days 为空，会先拉 `itinerary_days`，没有则回退 baseline，而不是整份重新生成。
- 打开行程时若 `generated` 标志丢失或只剩半成品天，不再把每日行程画成空白，也不会用半成品覆盖云端原行程；会显示已有天数，并在安静期结束后从 baseline 恢复。
...
- **全新一体化 Liquid Glass 胶囊输入栏**：将图片上传、多行自适应输入框与智能发送/状态药丸深度整合为统一的通透毛玻璃胶囊，完美匹配全局设计系统。
- **动态 Agentic 步骤感知胶囊**：底部状态药丸实时与后端流水线联动，生动呈现「规划策略中…」、「解析图片中…」、「搜索网络中…」、「深度思考中…」与「更新行程中…」，并支持丝滑的单元素 Spring 弹性拉伸与收拢动效。
- **全面支持 iPhone HEIC/HEIF 照片上传与并发解码**：
- 引入最新现代 `heic-to` 解码器（基于 Libheif WebAssembly），完美支持 iOS 16/17/18 HDR / Live Photo 照片转换；
- 支持最多同时附加 6 张图片，并在解码期间提供 1:1 紧凑型 Shimmer 骨架屏与居中旋转加载动画；
- 支持图片拖拽至面板全屏识别与剪贴板快捷粘贴。
- **通用视觉代理与非多模态模型兼容**：即使选用纯文本模型，系统亦能自动调用视觉代理提取图片实体并在行程对话中无缝理解。
- **通识泛知识问答支持**：优化提示词规则，当用户提问与行程无关的一般常识或日常话题时能够自然清晰作答。

### 变更

- 优化英文占位符文案为 `Ask or adjust your trip plan…`，彻底解决狭窄屏幕下的截断问题。
- 精细调优建议提示词胶囊文案长度，保持单行整齐展示与轻盈视觉。

### 修复

- 修复解析失败时的硬编码中文兜底，全面适配多语言。
- 修复发送按钮展开瞬间偶尔出现顶部粗白边的问题，并锁死纵向 32px 高度以消除垂直挤压伪影。
- 修复初次触发思考模型时状态时序短暂倒挂的偶现问题。
